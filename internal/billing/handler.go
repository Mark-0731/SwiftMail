...
func (h *Handler) GetPlans(c *fiber.Ctx) error {
	return response.OK(c, AvailablePlans)
}
...
func (h *Handler) StripeWebhook(c *fiber.Ctx) error {
	...
	event, err := h.service.stripe.ConstructWebhookEvent(payload, signature)
	if err != nil {
	...
	go func() {
		if err := h.service.HandleWebhook(c.Context(), event); err != nil {
			h.service.logger.Error().Err(err).Str("event_type", string(event.Type)).Msg("webhook processing failed")
		}
	}()
...
func (h *Handler) GetPublishableKey(c *fiber.Ctx) error {
	return response.OK(c, fiber.Map{
		"publishable_key": h.service.stripe.GetPublishableKey(),
	})
}
...
func (h *Handler) GetTransactions(c *fiber.Ctx) error {
	...
	rows, err := h.service.db.Query(c.Context(),
	...
	for rows.Next() {
		var tx Transaction
		if err := rows.Scan(&tx.ID, &tx.UserID, &tx.Amount, &tx.Type, &tx.Description, &tx.StripePaymentID, &tx.CreatedAt); err != nil {
			continue
		}
