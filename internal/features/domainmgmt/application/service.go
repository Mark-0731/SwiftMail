...
import (
	...
	"github.com/Mark-0731/SwiftMail/internal/features/domainmgmt/domain"
	"github.com/Mark-0731/SwiftMail/internal/features/domainmgmt/infrastructure"
...
type Service interface {
	AddDomain(ctx context.Context, userID uuid.UUID, req *domainmgmt.AddDomainRequest) (*domainmgmt.DomainResponse, error)
	ListDomains(ctx context.Context, userID uuid.UUID) ([]domainmgmt.DomainResponse, error)
	GetDomain(ctx context.Context, id uuid.UUID) (*domainmgmt.DomainResponse, error)
	VerifyDomain(ctx context.Context, id uuid.UUID) (*domainmgmt.DomainResponse, error)
...
func NewService(repo infrastructure.Repository, dns *domain.DNSChecker, rdb *redis.Client, logger zerolog.Logger) Service {
	return &service{repo: repo, dns: dns, rdb: rdb, logger: logger}
}

func (s *service) AddDomain(ctx context.Context, userID uuid.UUID, req *domainmgmt.AddDomainRequest) (*domainmgmt.DomainResponse, error) {
	domainName := validator.NormalizeDomain(req.Domain)
	if !validator.IsValidDomain(domainName) {
		return nil, fmt.Errorf("invalid domain name: %s", domainName)
	}

	// Check MX records
	hasMX, _, err := s.dns.CheckMX(ctx, domainName)
	if err != nil {
		s.logger.Warn().Err(err).Str("domain", domainName).Msg("MX check failed")
	}
	...
	spfRecord := domain.GenerateSPFRecord([]string{})
	dmarcRecord := domain.GenerateDMARCRecord(domainName)
	...
	s.logger.Info().Str("domain", domainName).Str("user_id", userID.String()).Msg("domain added")

	return s.toDomainResponse(model), nil
}

func (s *service) ListDomains(ctx context.Context, userID uuid.UUID) ([]domainmgmt.DomainResponse, error) {
	domains, err := s.repo.GetByUserID(ctx, userID)
	if err != nil {
		return nil, err
	}

	var resp []domainmgmt.DomainResponse
...
func (s *service) GetDomain(ctx context.Context, id uuid.UUID) (*domainmgmt.DomainResponse, error) {
...
func (s *service) VerifyDomain(ctx context.Context, id uuid.UUID) (*domainmgmt.DomainResponse, error) {
	d, err := s.repo.GetByID(ctx, id)
	if err != nil {
...
func (s *service) toDomainResponse(d *domainmgmt.Model) *domainmgmt.DomainResponse {
	resp := &domainmgmt.DomainResponse{
		ID:            d.ID,
		Domain:        d.Domain,
		Status:        d.Status,
	...
	if d.SPFRecord != nil {
		resp.DNSRecords = append(resp.DNSRecords, domainmgmt.DNSRecord{
			Type: "TXT", Name: domain.SPFRecordName(d.Domain), Value: *d.SPFRecord, Status: "pending",
		})
	}
	if d.DKIMSelector != nil && d.DKIMPublicKey != nil {
		resp.DNSRecords = append(resp.DNSRecords, domainmgmt.DNSRecord{
			Type: "TXT", Name: domain.DKIMRecordName(*d.DKIMSelector, d.Domain), Value: *d.DKIMPublicKey, Status: "pending",
		})
	}
	if d.DMARCRecord != nil {
		resp.DNSRecords = append(resp.DNSRecords, domainmgmt.DNSRecord{
			Type: "TXT", Name: domain.DMARCRecordName(d.Domain), Value: *d.DMARCRecord, Status: "pending",
		})
	}
