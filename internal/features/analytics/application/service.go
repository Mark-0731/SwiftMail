...
func (s *Service) TrackEvent(event analytics.Event) {
...
func (s *Service) GetOverview(ctx context.Context, userID uuid.UUID, from, to time.Time) (*analytics.Overview, error) {
...
func (s *Service) GetTimeSeries(ctx context.Context, userID uuid.UUID, from, to time.Time) ([]analytics.TimeSeriesPoint, error) {
	query := `
		SELECT
...
func (s *Service) GetTopDomains(ctx context.Context, userID uuid.UUID, from, to time.Time, limit int) ([]analytics.DomainStats, error) {
	query := `
		SELECT
	...
	var stats []analytics.DomainStats
	for rows.Next() {
		d := analytics.DomainStats{}
		var bounced int64
