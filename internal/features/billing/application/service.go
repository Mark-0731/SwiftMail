...
func (s *Service) GetSubscription(ctx context.Context, userID uuid.UUID) (*billing.Subscription, error) {
...
func (s *Service) handleSubscriptionUpdated(ctx context.Context, event stripe.Event) error {
	...
	if len(sub.Items.Data) > 0 {
		...
		starterPriceID := s.stripe.GetPriceID("starter")
		proPriceID := s.stripe.GetPriceID("pro")
...
func findPlan(id string) billing.Plan {
	for _, p := range billing.AvailablePlans {
		if p.ID == id {
			return p
		}
	}
	return billing.AvailablePlans[0] // default to free
}
...
func (s *Service) ConstructWebhookEvent(payload []byte, signature string) (stripe.Event, error) {
